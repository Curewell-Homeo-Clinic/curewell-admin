import { ClinicCreateForm } from "@/components/Form";
...
const ClinicTable: React.FC<{ clinics: InferQueryOutput<"clinic.getAll"> }> = ({
  clinics: allClinics,
}) => {
  ...
  const handleSearch = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    } else setClinics(allClinics);
  };

  const tds = clinics.map((clinic) => (
    <tr key={clinic.id}>
      <td className="select-none">{clinic.number}</td>
      <td className="capitalize">{clinic.name}</td>
      <td>{truncateString(clinic.address)}...</td>
      {isAdmin && (
