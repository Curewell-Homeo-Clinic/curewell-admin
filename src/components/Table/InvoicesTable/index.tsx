  ...
  const handleSearch = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (search !== "") {
      setInvoices(
        allInvoices.filter(
          (invoice) =>
            invoice.patient.firstName.includes(search) ||
            invoice.patient.lastName.includes(search)
        )
      );
    } else setInvoices(allInvoices);
  };

  const tds = invoices.map((invoice, index) => (
    <tr key={invoice.id}>
      <td className="select-none">{index + 1}</td>
      <td
        className="capitalize cursor-pointer hover:underline"
        onClick={() => handleViewDetail(invoice.patient.id, "patients")}
      >
      ...
      <td className="py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
        <button
          type="button"
          onClick={() => handleViewDetail(invoice.id, "invoices")}
